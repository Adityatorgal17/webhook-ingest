...
// InsertEvent stores the raw delivery unless it has already been seen.
// The atomic conflict handling keeps retries and concurrent deliveries safe.
...
// SaveIngest writes the event, call, and per-account aggregates as one atomic unit.
// If any step fails, the transaction is rolled back and no partial event state is left behind.
func (s *Store) SaveIngest(ctx context.Context, e Event) (bool, error) {
